...
We create a `CombinedTestResult` from this:
...
Since the *t* test is a continuous test, we do not need a special constructor to create a `CombinedTestResult`, but can use generic one using only the *p* value computed with an existing function, here `scipy.stats.ttest_rel`:
...
To do this, we only need to multiply the respective `CombinedTestResult` s:
...
if __name__ == "__main__":
	...
	# example-start
	from combine_pvalues_discrete import CombinedTestResult
	result_A = CombinedTestResult.from_sign_test(A_1,A_2,alternative="less")
	
	result_B = CombinedTestResult.from_mann_whitney_u(B_1,B_2,alternative="less")
